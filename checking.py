from typing import List, Dict, Union, Tuple, Optional, ForwardRef
...
# Forward references for types used within their own definitions
FigmaNode = ForwardRef('FigmaNode')
DSLNode = ForwardRef('DSLNode')
...
class FigmaNodeInner(TypedDict):
    ...
    children: Optional[List[FigmaNode]]  # Using ForwardRef for FigmaNode
...
class DSLNodeInner(TypedDict):
    ...
    cornerRadius: Optional[int]
    strokeWeight: Optional[int]
    children: Optional[List[DSLNode]]  # Using ForwardRef for DSLNode
    dropShadow: Optional[int]
